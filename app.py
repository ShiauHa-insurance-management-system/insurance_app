                ...
                if ext in ['png', 'jpg', 'jpeg']:
                    ...
                    with open(fpath, "rb") as f:
                        st.download_button(
                            ...
                            data=f, 
                            file_name=fn, 
                            key=f"dl_{prefix}_{idx}_{fn}",
                            use_container_width=True
                        )
    if not found:
        st.info("ℹ️ 暫無關聯附件 (檔名需含車牌或ID)")
...
with t_ren:
    render_list(st.session_state.renew_db, "被保險人", "牌照號碼", "被保險人ID", RENEW_FIELDS, "ren")

with t_pro:
    render_list(st.session_state.prog_db, "被保險人姓名", "牌照號碼", "被保險人身份證字號/統一編號", PROG_FIELDS, "prog")

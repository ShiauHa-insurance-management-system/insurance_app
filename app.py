...
def display_files(plate, uid, prefix, index):
    """依照車牌或 ID 自動搜尋 attachments 資料夾內的檔案並歸檔顯示"""
    ...
    if not found_any:
        st.caption("目前無關聯附件 (請確保檔名包含車牌或ID)")
...
with t_man:
    ...
    up_files = st.file_uploader("上傳檔案 (檔名請包含車牌或ID)", accept_multiple_files=True)
    if up_files:
        for f in up_files:
            with open(os.path.join(ATTACH_DIR, f.name), "wb") as sf: sf.write(f.getbuffer())
        st.success(f"已成功上傳 {len(up_files)} 個檔案"); st.rerun()

...
st.set_page_config(
    page_title="產險行動辦公室", 
    layout="wide", 
    initial_sidebar_state="collapsed" # 手機版預設收合選單，增加穩定性
)
...
if not st.session_state.auth:
    ...
    # 使用 form 包裹登入資訊，減少手機鍵盤彈出造成的刷新
    with st.form("login_form"):
        pwd = st.text_input("請輸入授權密碼", type="password")
        submit_login = st.form_submit_button("確認登入")
        if submit_login:
            if pwd == "1234":
...
with st.sidebar:
    st.header("➕ 行動手動錄入")
    target = st.selectbox("存入目標", ["續保明細", "出單進度"], key="sidebar_target")
    ...
    with st.form("sidebar_form", clear_on_submit=True):
        ...
        for f in fields:
            if "日" in f:
                entry[f] = str(st.date_input(f, key=f"input_{f}"))
            else:
                entry[f] = st.text_input(f, key=f"input_{f}")
...
if not urgent_list.empty:
    st.dataframe(urgent_list[['被保險人姓名', '牌照號碼', '到期日']], hide_index=True, use_container_width=True)
...
def show_ui(df, name_key, plate_key, id_key, fields, prefix):
    # 增加輸入框穩定性
    query = st.text_input(f"🔎 搜尋姓名/車牌/ID", key=f"search_{prefix}", placeholder="請輸入關鍵字...")
...
with t_man:
    ...
    u_r = st.file_uploader("更新續保 Excel", type="xlsx", key="upload_renew")
    if u_r:
        st.session_state.renew_db = pd.read_excel(u_r).fillna("")
        save_data(st.session_state.renew_db, DB_RENEW)
        st.success("續保資料已同步至伺服器"); st.rerun()
    
    u_p = st.file_uploader("上傳進度 Excel", type="xlsx", key="upload_prog")
    if u_p:
    ...
    st.divider()
    st.subheader("🖼️ 附件管理")
    up_files = st.file_uploader("批次上傳照片", accept_multiple_files=True, key="upload_files")
    if up_files:
        for f in up_files:

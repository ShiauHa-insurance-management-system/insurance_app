    ...
    if os.path.exists(ATTACH_DIR):
        for fn in os.listdir(ATTACH_DIR):
            if any(k in fn for k in m_keys):
                ...
                if ext in ['png', 'jpg', 'jpeg']: st.image(fpath)
                elif ext == 'pdf':
                    with open(fpath, "rb") as f:
                        st.download_button(f"📥 下載 {fn}", f, file_name=fn, key=f"{prefix}_{index}_{fn}")
...
if not p_urgent.empty or not r_remind.empty:
    with st.expander("🚨 到期作業提醒", expanded=True):
        if not p_urgent.empty:
            for _, r in p_urgent.iterrows(): st.error(f"📍今日/週末到期: {r['被保險人姓名']} ({r['牌照號碼']})")
        if not r_remind.empty:
            for _, r in r_remind.iterrows(): st.warning(f"⏳續保預警(2月內): {r['被保險人']} ({r['牌照號碼']})")

t_ren, t_pro, t_man = st.tabs(["🔍 續保", "📑 進度", "⚙️ 管理"])

def show_list(df, name_k, plate_k, id_k, fields, prefix):
    q = st.text_input(f"🔎 搜尋", key=f"s_{prefix}", placeholder="姓名/車牌/ID...")
...
with t_man:
    ...
    if u_r: st.session_state.renew_db = pd.read_excel(u_r).fillna(""); save_data(st.session_state.renew_db, DB_RENEW); st.rerun()
    u_p = st.file_uploader("更新進度 Excel", type="xlsx")
    if u_p: st.session_state.prog_db = pd.read_excel(u_p).fillna(""); save_data(st.session_state.prog_db, DB_PROG); st.rerun()
    st.divider()
    up = st.file_uploader("上傳附件", accept_multiple_files=True)
    if up:
        for f in up:
            with open(os.path.join(ATTACH_DIR, f.name), "wb") as sf: sf.write(f.getbuffer())
        st.success("上傳成功"); st.rerun()
